...
public class Ghost : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Collider GameManager = pacman.GetComponent<Collider>();


        Vector3 source = new Vector3((-create_maze.bound(0) / 2) + i, 1, (-create_maze.bound(1) / 2) + j);
        Vector3 dest = new Vector3((-create_maze.bound(0) / 2) + ni, 1, (-create_maze.bound(1) / 2) + nj);
        ...
        if (distCovered >= 1.0f) // Já chegou no destino
        {
            ...
            int ti = 0, tj = 0;
            float distance = 100;
            Vector3 search = new Vector3((-create_maze.bound(1) / 2) + ni, 1, (-create_maze.bound(1) / 2) + nj);

            foreach (int[] point in connected4)
            {
                if ((create_maze.map[ni + point[0], nj + point[1]] < 8) && !(i == ni + point[0] && j == nj + point[1]))
                {
